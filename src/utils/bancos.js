/**
 * Catálogo de bancos emissores (somente UI + slug persistido no cartão).
 * Expansível adicionando entradas em BANCOS_CATALOGO.
 */
...
export function listarBancos() {
  return BANCOS_CATALOGO.filter((b) => b.slug !== 'outro');
}
...
export function resolverBancoOutro() {
  return MAPA_BANCOS.outro;
}

/**
 * Tenta inferir banco a partir do nome livre do cartão (cartões legados).
 */
...
/**
 * Resolve banco do cartão: slug salvo ou inferência pelo nome.
 */
...
export function obterNomeExibicaoCartao(cartao) {
  const apelido = String(cartao?.nome ?? '').trim();
  const banco = resolverBancoParaCartao(cartao);
  return apelido || banco?.nome || 'Sem nome';
}
